...
DO NOT: Create ad-hoc modal styles. Use the **Unified Modal System** (BeeClass Rule):
  - Tokens: Always use `--modal-bg`, `--modal-radius` (2xl), `--modal-backdrop`.
  - Anatomy: Consistent header with title + subtext, footer with primary action on the right.
  - Component: Always extend or use `UnifiedModal`.
