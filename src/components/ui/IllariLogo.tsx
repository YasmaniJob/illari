...
export default function IllariLogo({ size = 'md' }: IllariLogoProps) {
  const s = sizes[size];
  return (
    <span className={`flex items-center gap-3 ${s.text}`}>
      ...
      <span className="font-extrabold text-warm-900 tracking-tight">Illari</span>
    </span>
  );
}
