    ...
    <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6 md:px-8">
      {messages.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          <span className="flex h-20 w-20 items-center justify-center rounded-3xl bg-honey-200 text-4xl mb-6">
            🌼
          </span>
          <p className="text-2xl font-extrabold text-warm-900">
            Tu línea de observaciones
          </p>
        ...
        msg.type === 'user' ? (
          <div key={msg.id} className="flex justify-end">
            <div className="max-w-[90%] sm:max-w-lg">
              {msg.studentName && (
                <p className="text-base font-bold text-warm-700 mb-2 text-right">
                  Sobre {msg.studentName}
                </p>
              )}
              ...
              <time className="mt-2 block text-base font-semibold text-warm-500 text-right">
                {msg.timestamp}
              </time>
            </div>
          </div>
