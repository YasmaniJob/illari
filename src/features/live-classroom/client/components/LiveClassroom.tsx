...
import ChatFeed from '@/features/live-classroom/client/components/ChatFeed';
import ChatInputBar from '@/features/live-classroom/client/components/ChatInputBar';
import CuadernoCampo from '@/features/live-classroom/client/components/CuadernoCampo';
import EditSessionDrawer from '@/features/live-classroom/client/components/EditSessionDrawer';
import PropositoModal from '@/features/live-classroom/client/components/PropositoModal';
import StudentList from '@/features/live-classroom/client/components/StudentList';
import StudentPickerSheet from '@/features/live-classroom/client/components/StudentPickerSheet';
import {
  useAulaAddObservationMutation,
  useAulaInit,
  useAulaPatchAIFieldMutation,
} from '@/features/live-classroom/client/hooks/useAula';
import QueryProvider from '@/shared/client/ui/QueryProvider';

function LiveClassroomContent() {
  const queryClient = useQueryClient();
  const { data, isLoading, error: initError } = useAulaInit();
  const { session, messages, students, loggedIn } = data || {
    session: null,
    messages: [],
    students: [],
    loggedIn: false,
  ...
  const addObsMutation = useAulaAddObservationMutation();
  const patchAIFieldMutation = useAulaPatchAIFieldMutation();

  const sending = addObsMutation.isPending;
  const error =
    localError || initError?.message || addObsMutation.error?.message || patchAIFieldMutation.error?.message;

  // Auto-select first student if none selected and students loaded
  useEffect(() => {
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      setLocalError(null);
      try {
        const res = await addObsMutation.mutateAsync({
        ...
        if (res.studentNameMatch) {
          const matched = students.find((s) => s.name.toLowerCase() === res.studentNameMatch!.toLowerCase());
          if (matched) {
    ...
    [session, selectedStudent, students, addObsMutation],
  );
  ...
  const handleUpdateAI = useCallback(
    async (
      ...
      try {
        await patchAIFieldMutation.mutateAsync({ id, field, value });
      } catch {
        /* Se maneja el error global en el markup */
      }
    },
    [patchAIFieldMutation],
  );
      ...
      <div className="flex flex-1 min-h-0 md:grid md:grid-cols-[300px_1fr]">
        ...
        <div className="flex flex-col min-h-0">
          <ChatFeed
            messages={messages}
            sending={sending}
            filterStudentName={selectedStudent?.name}
